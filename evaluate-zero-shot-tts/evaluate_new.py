...
from src.evaluate_zero_shot_tts.utils.model_factory import get_inference
...
evaluator_dict = {
    "hubert": {
        ...
        "model_type": "../pretrained/evaluation/hubert-large-ls960-ft",
        "lang": "en",
        "name": "wer",
        "amp": False,
        "sdpa_kernel": False,
    ...
    "conformer": {
        ...
        "device": "cuda:0",
        "org_sample_rate": None,  # will be automatically detected
        "tar_sample_rate": 16000,  # conformer is trained with 16k
        "model_type": "../pretrained/evaluation/stt_en_conformer_transducer_xlarge.nemo",
        "lang": None,
        "name": "wer",
        "amp": False,
        "sdpa_kernel": False,
    ...
    "whisper": {
        ...
        "model_type": "../pretrained/evaluation/whisper-large-v2.pt",
        "lang": None,
        "name": "wer",
        "options": {
            "language": "english",
    ...
    "wavlmuni": {
        ...
        "model_type": "../pretrained/evaluation/wavlm_large_finetune.pth",
        "lang": None,
        "name": "sim",
        "amp": False,
        "sdpa_kernel": False,
...
def main(args):
    ...
    # set directory path
    dataset_name, exp_id, model_name, task, model_id, ckpt_name = root_dir.split("/")[-6:]
    num_trials = int(exp_id.split("_r")[1][0])
    evalset_dir = os.path.join("evalsets", dataset_name, exp_id)
    out_dir = os.path.join(args.out_dir, dataset_name, model_name, exp_id, task, model_id, ckpt_name)
    ...
    if args.metric in ["wer", "cer"]:
        assert args.evaluator in ["hubert", "whisper", "conformer"]
    ...
    def extract_sort_key(filename):
        ...
        # Use regex to extract all consecutive numeric parts in the filename
        numbers = re.findall(r'\d+', base_name)
        
        # Convert extracted strings to a list of integers
        int_parts = [int(num) for num in numbers]
        return int_parts
    ...
    for i, wav_path in tqdm.tqdm(
        ...
        if inference_task != "wav_g":
            ...
            for i in range(num_trials):
                wav_p_path = wav_path.replace("wav_g.wav", f"{wav_g_task}_{i}.wav")
                evaluator = get_res(
                    evaluator, wavs_hat, wav_lens_hat, wav_path, wav_p_path, sr, wo_dup_prmpt=task == "cont" and "sim" in args.metric
                )

    from src.evaluate_zero_shot_tts.utils.metric_stats import get_metric
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--root_dir",
        type=str,
        default="samples/librispeech-test-clean/exp_base_pl3_r3/melle/cont/belle-lr5e-4-kl0-edl0.1-flux0-epoch150-max14-scheduler_linear-librispeech-AdamW-pos_learn_True-vocoder_speechtts_pkl-g2p_espeak/epoch-150",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=["hubert", "conformer", "whisper", "wavlmuni"],
    )
    parser.add_argument(
        "-m",
        "--metric",
