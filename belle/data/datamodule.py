...
from belle.data.dataset import SpeechSynthesisDataset
...
class TtsDataModule:
    ...
    def add_arguments(cls, parser: argparse.ArgumentParser):
        ...
        group.add_argument(
            ...
            type=str,
            nargs="+",
            help="TTS models' data to use. "
        )
        group.add_argument(
            "--lazy-load",
            type=str2bool,
            default=True,
            help="When enabled, use lazy loading for cuts. "
            "This is useful for large datasets to save memory.",
        )
    ...
    def train_dataloaders(
        ...
        if self.args.concatenate_cuts: # false
            logging.info(
        ...
        input_transforms = []
        if self.args.enable_spec_aug: # false
        ...
        if self.args.on_the_fly_feats: # false
            ...
            train = SpeechSynthesisDataset(
                ...
                cut_transforms=transforms,
                # feature_transforms=input_transforms,
                tts_models=self.args.tts_models,
            )

        if self.args.bucketing_sampler: # true
    ...
    def train_cuts(self) -> CutSet:
        logging.info(f"About to get train cuts: {self.args.train_cuts_path}")
        if self.args.lazy_load:
            # Use lazy loading for large datasets to save memory.
            # This is useful for large datasets like libriheavy.
            return load_manifest_lazy(self.args.train_cuts_path)
        else:
            return CutSet.from_jsonl(self.args.train_cuts_path)

    # @lru_cache()
    # def dev_cuts(self) -> CutSet:
    #     cuts_path = "data/tokenized/cuts_dev.jsonl.gz"
    #     logging.info(f"About to get dev cuts: {cuts_path}")
    #     if self.args.lazy_load:
    #         # Use lazy loading for large datasets to save memory.
    #         # This is useful for large datasets like libriheavy.
    #         return load_manifest_lazy(cuts_path)
    #     else:
    #         return CutSet.from_jsonl(cuts_path)

    # @lru_cache()
    # def test_cuts(self) -> CutSet:
    #     cuts_path = "data/tokenized/cuts_test.jsonl.gz"
    #     logging.info(f"About to get test cuts: {cuts_path}")
    #     return load_manifest_lazy(cuts_path)
