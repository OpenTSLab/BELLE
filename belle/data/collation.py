...
from belle.utils import SymbolTable
...
class TextTokenCollater:
    ...
    def __init__(
        ...
        print(f'Token2idx: {self.token2idx}')
        self.idx2token = [token for token in unique_tokens]

    def _token2idx(self, token: str) -> int:
    ...
    def index(
        ...
        tokens = torch.from_numpy(
            np.array(
                [[self._token2idx(token) for token in seq] for seq in seqs],
                dtype=np.int64,
            )
        )
    ...
    def __call__(self, texts: List[str], pad: bool = True) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # tokens_batch is a list[Tensor]
            ...
            tokens_batch = [
                torch.IntTensor([self._token2idx(token) for token in seq])
                for seq in seqs
            ]
...
if __name__ == "__main__":
    text_token_file = "egs/librispeech/data/tokenized/unique_text_tokens.k2symbols"
    collater = get_text_token_collater(text_token_file)

    # python -m belle.data.collation
